...
async def practice_next(req: PracticeNextRequest, db: AsyncSession = Depends(get_db)):
    ...
    candidates = []
    for s in skills:
        due = (s.next_due is None) or (s.next_due <= now)
        sc = score_candidate(due=due, strength=s.strength, next_due=s.next_due)
        candidates.append((sc, due, s))

    candidates.sort(key=lambda x: x[0])

    due_list = []
    weak_list = []
    for sc, due, s in candidates[: max(req.limit, 10)]:
        out = SkillOut(skill_id=s.skill_id, strength=s.strength, next_due=s.next_due, streak=s.streak, mistakes=s.mistakes)
    ...
    return PracticePlan(due=due_list, weak=weak_list, new=new_skills, scenario_prompt=scenario)
